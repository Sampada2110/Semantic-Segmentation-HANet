...
def make_cv_splits(img_dir_name):
    ...
    split0 is aligned with the default ACDC train/val.
    """
    trn_path = os.path.join(root, img_dir_name, 'rgb_anon', 'train')
    val_path = os.path.join(root, img_dir_name, 'rgb_anon', 'val')
...
def make_test_split(img_dir_name):
    test_path = os.path.join(root, img_dir_name, 'rgb_anon', 'test')
...
class AcdcUniform(data.Dataset):
    ...
    def __init__(self, quality, mode, maxSkip=0, joint_transform_list=None, sliding_crop=None,
        ...
        json_fn = 'acdc{}_cv{}_tile{}.json'.format(
        ...
        if self.maxSkip > 0:
            json_fn = 'acdc{}_cv{}_tile{}_skip{}.json'.format(
        ...
        if self.coarse_boost_classes is not None:
            json_fn = 'acdc_coarse_{}_tile{}.json'.format(
...
class AcdcUniformWithPos(data.Dataset):
    
    def __init__(self, quality, mode, maxSkip=0, joint_transform_list=None, sliding_crop=None,
                 transform=None, target_transform=None, target_aux_transform=None, dump_images=False,
                 cv_split=None, class_uniform_pct=0.5, class_uniform_tile=1024,
                 test=False, coarse_boost_classes=None, pos_rfactor=8):
        print("######## AcdcUniformWithPos #########")
        self.quality = quality
        self.mode = mode
        self.maxSkip = maxSkip
        self.joint_transform_list = joint_transform_list
        ...
        # Add in coarse centroids for certain classes
        if self.coarse_boost_classes is not None:
            json_fn = 'acdc{}_tile{}.json'.format(
                self.mode, self.class_uniform_tile)
            if os.path.isfile(json_fn):
                with open(json_fn, 'r') as json_data:
                    centroids = json.load(json_data)
...
class AcdcWithPos(data.Dataset):
